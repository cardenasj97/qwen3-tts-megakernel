    ...
    if (threadIdx.x == 0) {
      ...
      if (arrived == nblocks - 1) {
        *counter = 0;
        ...
        // Blackwell (sm_120) L1 may not see cross-SM writes via volatile.
        // Use acquire load for coherent observation of the generation counter.
        unsigned int observed;
        do {
          asm volatile("ld.global.acquire.gpu.u32 %0, [%1];"
                       : "=r"(observed) : "l"(generation));
        } while (observed <= my_gen);
      }
      local_gen = my_gen + 1;
    }
    __syncthreads();
...
#pragma unroll
    ...
    if (block_id > 0 && block_id < ATTN_BLOCKS) {
      if (threadIdx.x == 0) {
        ...
        do {
          asm volatile("ld.global.acquire.gpu.u32 %0, [%1];"
                       : "=r"(kv_obs) : "l"(kv_flag));
        } while (kv_obs < (unsigned int)(layer_idx + 1));
      }
      __syncthreads();
    }
  ...
  if (attn_flag) {
    ...
    if (threadIdx.x == 0) {
      ...
      unsigned int attn_obs;
      do {
        asm volatile("ld.global.acquire.gpu.u32 %0, [%1];"
                     : "=r"(attn_obs) : "l"(attn_flag));
      } while (attn_obs < target);
    }
    __syncthreads();
  } else {
    grid.sync();
  ...
  if (blockIdx.x == 0) {
    if (threadIdx.x == 0) {
      ...
      do {
        asm volatile("ld.global.acquire.gpu.u32 %0, [%1];"
                     : "=r"(lm_obs) : "l"(counter));
      } while (lm_obs < (unsigned int)num_blocks);
      __threadfence();
    }
    __syncthreads();
...
// =============================================================================
// Decode from pre-computed embeddings (for TTS talker integration)
// =============================================================================

__global__ void __launch_bounds__(LDG_BLOCK_SIZE, 1) ldg_decode_kernel_from_embeds(
    const __nv_bfloat16 *__restrict__ input_embeds,  // pre-computed (HIDDEN_SIZE,) bf16
    const LDGLayerWeights *__restrict__ layer_weights,
    const __nv_bfloat16 *__restrict__ final_norm_weight,
    const __nv_bfloat16 *__restrict__ cos_table,
    const __nv_bfloat16 *__restrict__ sin_table,
  ...
  // KEY CHANGE: use caller-provided embedding directly instead of table lookup
  const __nv_bfloat16 *embed_row = input_embeds;
...
extern "C" void launch_ldg_decode_direct(
  ...
  // Host-side barrier reset: ensures clean state for Blackwell (sm_120) where
  // in-kernel block-0 reset races with other blocks reading stale values.
...
extern "C" void launch_ldg_decode_from_embeds(
  ...
  // Host-side barrier reset (see launch_ldg_decode_direct comment)
  cudaMemsetAsync(d_barrier_counter, 0, sizeof(unsigned int), stream);
  cudaMemsetAsync(d_barrier_sense, 0, sizeof(unsigned int), stream);
  cudaMemsetAsync(d_kv_flag, 0, sizeof(unsigned int), stream);
  cudaMemsetAsync(d_attn_flag, 0, sizeof(unsigned int), stream);

  ldg_decode_kernel_from_embeds<<<LDG_NUM_BLOCKS, LDG_BLOCK_SIZE, 0, stream>>>(
      (const __nv_bfloat16 *)input_embeds, layer_weights,
      (const __nv_bfloat16 *)final_norm_weight,
      (const __nv_bfloat16 *)cos_table, (const __nv_bfloat16 *)sin_table,
      (__nv_bfloat16 *)k_cache, (__nv_bfloat16 *)v_cache,
      (__nv_bfloat16 *)hidden_buffer, (float *)g_activations,
